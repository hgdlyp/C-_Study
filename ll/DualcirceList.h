...
namespace DTLib
{
	...
	protected:
		...
		list_head m_header;
		list_head * m_current;
		list_head *position(int i) const   //定位函数 
		{
			list_head *ret = const_cast<list_head *>(&m_header);
			for(int p=0;p<i;p++)   //定位到i所在的位置 
			{
				ret = ret->next;
			}
			return ret;
	...
	public:
	 	DualcircleList()  //构造函数即是对一些成员变量进行初始化 
	 	{
	 		this->m_length = 0;
	 		this->m_step = 1;
		 ...
		 bool insert(int i,const T &e) 
		 {
		 	...
		 	{
		 		 node->value = e;
				 list_add_tail(&node->head,this->position(i)->next);  //从尾部插入链表元素 
				 this->m_length++;
			} 
			else
			{
		...
		bool set( int i, const T&e)	
			...
			{
		    	int i = 0;
			 	while ( !end() && (i<this->m_step))//还没到达链表的尾部 
					{
			...
			bool pre()
			{
				int i = 0;
			 	while ( !end() && (i<this->m_step))//还没到达链表的尾部 
					{
						if(m_current != &m_header)   //判断是否等于头结点，如果是则跳过 
						{
